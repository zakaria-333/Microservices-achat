...
const Row1 = () => {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const [demandeurResponse, demandeResponse] = await Promise.all([
          axios.get(`/api/user/demandeurBySite/${user.site.id}`, {
        ...
        // Set demandeur count
        setDemandeurCount(demandeurResponse.data.length);
        ...
        console.log('Demandeur Count:', demandeurResponse.data.length);
    ...
    fetchCounts();
  }, [token, user.site.id]); // Added user.site.id as a dependency
    ...
    <Stack direction={'row'} sx={{ justifyContent: 'center', mt: '30px' }}>
      <Paper sx={{ minWidth: '18%', mr: '20px', p: 1.5, display: 'flex', justifyContent: 'space-between' }}>
        <Stack gap={1}>
          ...
          <Typography sx={{ fontSize: '20px' }}>Demandeurs</Typography>
        </Stack>
        <Stack gap={1}>
          <Typography sx={{ fontSize: '30px', m: '20px' }}>{`+${demandeurCount}`}</Typography>
        </Stack>
      </Paper>
      <Paper sx={{ minWidth: '18%', mr: '20px', p: 1.5, display: 'flex', justifyContent: 'space-between' }}>
        <Stack gap={1}>
