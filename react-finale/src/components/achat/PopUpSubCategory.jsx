...
export default function PopUp({ action, data,onSuccess }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const url = action === 'Update' ? `/api/budget/subcategorie/update` : '/api/budget/subcategorie/create';
    ...
    try {
      const response = await axios({
        ...
        data: {
          ...
          budget// Ensure `filiale` is sent as an object with `id`
        }
      });
  ...
  return (
    <React.Fragment>
      ...
      {action == 'Add SubCategory' && <Button onClick={handleClickOpen} sx={{width:'300px',margin:1,fontSize:'16px',fontFamily:'initial',bgcolor:theme.palette.mode=='dark'?'white':'black',color:theme.palette.mode=='dark'?'black':'white','&:hover': {
