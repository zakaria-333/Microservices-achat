...
export default function PopUp({ action, data, onSuccess }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const url = action === 'Update' ? `/api/budget/categorie/update` : '/api/budget/categorie/create';
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({ id, nom,budget, code })
      });
...
// popUp affiche pour soit ajouter ou modifier un filiale 
  ...
  return (
    <React.Fragment>
      ...
      {action == 'Add Category' && <Button onClick={handleClickOpen} sx={{width:'280px',margin:1,fontSize:'16px',fontFamily:'initial',bgcolor:theme.palette.mode=='dark'?'white':'black',color:theme.palette.mode=='dark'?'black':'white','&:hover': {
