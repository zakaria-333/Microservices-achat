...
import SnackNar from '../SnackNar';
import PersonSearchOutlinedIcon from '@mui/icons-material/PersonSearchOutlined';
import Person3OutlinedIcon from '@mui/icons-material/Person3Outlined';
import CalendarMonthOutlinedIcon from '@mui/icons-material/CalendarMonthOutlined';
import CategoryOutlinedIcon from '@mui/icons-material/CategoryOutlined';
import PopUpValider from './PopUpValider';

const Carde = ({ code, name, date, desc, prop, etat, sub, cat, demande,fetch,catId }) => {
  const theme = useTheme();
  const { user, token,demandesCount,fetchDemandesAchat } = useContext(AuthContext);
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      alert('Impossible de soumettre la justification');
    }
  };
      ...
      <React.Fragment>
        <CardContent>
          ...
          >
            ...
            <Stack direction={'row'} sx={{ justifyContent: 'space-between',mt:'8px',color:'black'  }}>
            <Typography sx={{fontWeight: 'bold' }} ><PersonSearchOutlinedIcon sx={{mr:'8px',fontWeight: 'bold' }} />Propriétaire</Typography>
