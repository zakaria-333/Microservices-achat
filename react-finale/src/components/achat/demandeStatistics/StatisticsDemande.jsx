      ...
      try {
        ...
        const categoryData = await Promise.all(
          categories.map(async (category) => {
            // Fetch the number of demandes for each category
            const demandesResponse = await fetch(`/api/demande/byCategorie`, {
              method: 'POST',
              headers: {
            ...
            if (!demandesResponse.ok) {
              throw new Error(`Failed to fetch demandes for category ${category.nom}`);
            }

            const demandes = await demandesResponse.json();

            // Return the category with the count of demandes
            return {
              id: category.nom, // Use category name as the id for the pie chart
              label: category.nom,
              value: demandes.length, // Number of demandes for this category
            };
          })
        );
  ...
  return (
    <Box sx={{ height: '70vh', paddingBottom: '80px' }}>
      <h1 className="text-center font-bold text-xl underline">
        Statistiques du nombre de demandes par catégorie
      </h1>
      <ResponsivePie
        data={data}
